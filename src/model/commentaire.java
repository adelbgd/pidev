/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package model;

import java.util.Date;

/**
 *
 * @author azizl
 */
public class commentaire {
     private int id_com;
    private int id_pub;
    private int id_user;
    private String username;
    private String suj_com;
    private Date date_com;
    private int nb_reaction;

    public commentaire() {
    }

    public commentaire(int id_com, int id_pub, int id_user, String username, String suj_com, Date date_com, int nb_reaction) {
        this.id_com = id_com;
        this.id_pub = id_pub;
        this.id_user = id_user;
        this.username = username;
        this.suj_com = suj_com;
        this.date_com = date_com;
        this.nb_reaction = nb_reaction;
    }

    public int getId_com() {
        return id_com;
    }

    public void setId_com(int id_com) {
        this.id_com = id_com;
    }

    public int getId_pub() {
        return id_pub;
    }

    public void setId_pub(int id_pub) {
        this.id_pub = id_pub;
    }

    public int getId_user() {
        return id_user;
    }

    public void setId_user(int id_user) {
        this.id_user = id_user;
    }

    public String getUsername() {
        return username;
    }

    public void setUsername(String username) {
        this.username = username;
    }

    public String getSuj_com() {
        return suj_com;
    }

    public void setSuj_com(String suj_com) {
        this.suj_com = suj_com;
    }

    public Date getDate_com() {
        return date_com;
    }

    public void setDate_com(Date date_com) {
        this.date_com = date_com;
    }

    public int getNb_reaction() {
        return nb_reaction;
    }

    public void setNb_reaction(int nb_reaction) {
        this.nb_reaction = nb_reaction;
    }

    @Override
    public String toString() {
        return "commentaire{" + "id_com=" + id_com + ", id_pub=" + id_pub + ", id_user=" + id_user + ", username=" + username + ", suj_com=" + suj_com + ", date_com=" + date_com + ", nb_reaction=" + nb_reaction + '}';
    }

    public commentaire(String suj_com, Date date_com, int nb_reaction) {
        this.suj_com = suj_com;
        this.date_com = date_com;
        this.nb_reaction = nb_reaction;
    }
    
}
